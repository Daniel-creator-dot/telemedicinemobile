...
class BookAppointmentDialog extends StatefulWidget {
  const BookAppointmentDialog({
    super.key,
    this.initialDoctor,
    this.initialTelemedicine,
  });

  final AuthUser? initialDoctor;
  final bool? initialTelemedicine;
...
class _BookAppointmentDialogState extends State<BookAppointmentDialog> {
  ...
  void _loadDoctorsAndSession() {
    ...
    if (session.isAuthenticated && session.user != null) {
      ...
      _isTelemedicine = widget.initialTelemedicine ?? true;
    } else if (widget.initialTelemedicine != null) {
      _isTelemedicine = widget.initialTelemedicine!;
    }
    ...
    context.read<AppointmentsRepository>().getAvailableDoctors().then((list) {
      ...
      if (initial != null) {
        selected = list.where((d) => d.id == initial.id).firstOrNull ?? initial;
      }
      setState(() {
        _doctors = list;
